...
def origin_render_path_change_loc():  # 将blender默认输出存到垃圾输出内，应在最后调用
    ...
    if False: #addon_prefs.Put_Default_To_trash_output:
        if current_render_path[-1:] != "\\":
            ...
            for node in bpy.context.scene.node_tree.nodes:
                if node.type == "R_LAYERS" and node.layer == view_layer:
                    ...
                    if bpy.context.scene.IDS_UsedN is True:
                        if True :#addon_prefs.Denoise_Col is True:
        ...
        for view_layer in viewlayers:
            ...
            if (
                ...
                for node in set(viewlayer_full[f"{view_layer}Data"]) - set(
                    ...
                    if node != "Vector":
                        nodes_to_check = ["Depth", "Mist"]
                        if any(node == x for x in nodes_to_check):
                            scene.node_tree.links.new(
                                scene.node_tree.nodes[f"{view_layer}"].outputs[f"{node}"],
                            ...
                            combine_xyz_node = scene.node_tree.nodes.new("CompositorNodeCombineXYZ")
                            combine_xyz_node.name = f"{view_layer}--{node}_Combine"
                            combine_xyz_node.label = f"{view_layer}_{node}_COMBINE"

                            scene.node_tree.links.new(
                                combine_xyz_node.outputs["Vector"],
                                scene.node_tree.nodes[f"{view_layer}--DaTA"].inputs[node],
                                )
                            scene.node_tree.links.new(
                                    scene.node_tree.nodes[f"{view_layer}"].outputs[node],
                ...
                for node1 in bpy.context.scene.node_tree.nodes:
                    if (
                        ...
                        for slot in node1.layer_slots:
                            ...
                            slot.name = slot.name.replace("Depth", "depth")
                            slot.name = slot.name.replace("Position", "position")
...
def auto_arr_mathnode():  # 排列数学运算节点
    ...
    for view_layer in viewlayers:
        for node in bpy.context.scene.node_tree.nodes:
            if node.type == "R_LAYERS" and node.layer == view_layer:
                counter = node.location.y-node.dimensions.y/2
                for node2 in reversed(bpy.context.scene.node_tree.nodes):
                    nodes_to_affect = ['Depth', 'Mist']
                    if (
                        node2.name[: node2.name.rfind("--")] == node.layer
                        and node2.type == "COMBINE_XYZ" and any(x in node2.name for x in nodes_to_affect)
                    ):
                        counter = counter+45
                        node2.location = 660, counter
                        node2.hide = True
